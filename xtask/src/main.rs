...
"#
        ...
        pub fn tarpaulin(args: &mut pico_args::Arguments, cargo_args: &[std::ffi::OsString]) -> crate::Fallible<()> {
            let help = r#"
xtask-tarpaulin
...
"#
            ...
            cmd.args(&["--out", "Xml"]);
            cmd.args(&["--packages", "xtask", "lsp-text"]);
            cmd.args(&["--exclude-files", "xtask"]);
            cmd.args(cargo_args);
            cmd.status()?;
